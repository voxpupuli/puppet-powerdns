    ...
    on_supported_os.each do |os, facts|
      context "on #{os}" do
        ...
        when 'Debian'
          ...
          mysql_schema_file = '/usr/share/pdns-backend-mysql/schema/schema.mysql.sql'
          pgsql_backend_package_name = 'pdns-backend-pgsql'
          pgsql_schema_file = '/usr/share/pdns-backend-pgsql/schema/schema.pgsql.sql'
          sqlite_backend_package_name = 'pdns-backend-sqlite3'
          sqlite_schema_file = '/usr/share/pdns-backend-sqlite3/schema/schema.sqlite3.sql'
        ...
        context 'powerdns class with parameters' do
          ...
          it { is_expected.to contain_file(authoritative_config).with(group: 'pdns') } if facts[:os]['family'] == 'RedHat'
        end
